...
AAimSkill2Projectile::AAimSkill2Projectile()
{
	...
	MeshComponent = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("MeshComponent")); // 스태틱 메쉬 컴포넌트 생성
	...
	InitialLifeSpan = 10.0f; // 액터 초기수명 설정
}
...
void AAimSkill2Projectile::EndPlay(const EEndPlayReason::Type EndPlayReason) // 액터가 제거될 때 호출되는 함수
{
...
void AAimSkill2Projectile::Tick(float DeltaTime)
{
	...
	if (!bHasReachedApex && GetActorLocation().Z >= ApexHeight) // 아직 정점에 도달하지 않았고 위치가 최고점 이상이라면
	{
		bHasReachedApex = true; // 최고점 도달 플래그 설정
		ProjectileMovement->StopMovementImmediately(); // 이동 즉시 정지
		GetWorldTimerManager().SetTimer(DelayTimerHandle, this, &AAimSkill2Projectile::EvaluateTargetAfterApex, DelayBeforeTracking, false); // 정점 후 타겟 평가 함수 호출 타이머 설정
	}
...
void AAimSkill2Projectile::FireInDirection(const FVector& ShootDirection)
{
	...
	{
		ProjectileMovement->Velocity = ShootDirection * ProjectileMovement->InitialSpeed; // 주어진 방향으로 속도 설정
	}
}
...
void AAimSkill2Projectile::AutoExplodeIfNoTarget()
{
	...
	OnHit(nullptr, nullptr, nullptr, FVector::ZeroVector, FHitResult()); // 충돌 이벤트 강제 호출
}

void AAimSkill2Projectile::OnHit(UPrimitiveComponent* HitComponent, AActor* OtherActor,
	UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	if (OtherActor && (OtherActor == this || OtherActor == Shooter)) return; // 자기 자신이나 발사자와 충돌 시 무시
	...
	if (FlightAudioComponent) FlightAudioComponent->Stop(); // 비행 사운드 정지
...
void AAimSkill2Projectile::ApplyPersistentEffects()
{
	...
	for (AActor* Actor : OverlappedActors)
	{
		if (Actor && Actor != Shooter)
		{
			...
			if (Enemy)
			{
				Enemy->bIsTrappedInGravityField = true; // 적이 중력장에 갇힘 True
