...
private:
    ...
    UPROPERTY()
	TObjectPtr<AEnemyShooter> CachedShooter = nullptr; // 제어 중인 슈터 캐싱

	UPROPERTY()
	TObjectPtr<AEnemyGuardian> CachedGuardian = nullptr; // 제어 중인 가디언 캐싱

    float LastShootTime = 0.0f; // 마지막 사격 시간 (쿨타임 계산용)

    UPROPERTY(EditDefaultsOnly, Category = "AI Combat", meta = (AllowPrivateAccess = "true"))
    ...
    UPROPERTY(EditDefaultsOnly, Category = "AI State", meta = (AllowPrivateAccess = "true"))
    EEnemyShooterAIState CurrentState = EEnemyShooterAIState::Idle; // 현재 AI 상태

    // AI 기본 설정 (거리)
    UPROPERTY(EditDefaultsOnly, Category = "AI Range", meta = (AllowPrivateAccess = "true"))
    ...
    // 포메이션(진형) 설정
    UPROPERTY(EditDefaultsOnly, Category = "AI Formation", meta = (AllowPrivateAccess = "true"))
    ...
    // 회전 제한 설정 (상하 시야각 제한)
    UPROPERTY(EditDefaultsOnly, Category = "AI Rotation", meta = (ClampMin = "-89.0", ClampMax = "0.0"), meta = (AllowPrivateAccess = "true"))
    float MaxLookDownAngle = -45.0f; // 아래쪽으로 볼 수 있는 최대 각도

    UPROPERTY(EditDefaultsOnly, Category = "AI Rotation", meta = (ClampMin = "0.0", ClampMax = "89.0"), meta = (AllowPrivateAccess = "true"))
    float MaxLookUpAngle = 15.0f; // 위쪽으로 볼 수 있는 최대 각도

    UPROPERTY(EditDefaultsOnly, Category = "AI Rotation", meta = (ClampMin = "1.0", ClampMax = "20.0"), meta = (AllowPrivateAccess = "true"))
    float RotationInterpSpeed = 8.0f; // 플레이어를 향해 회전하는 속도

    // 성능 최적화 설정
    UPROPERTY(EditDefaultsOnly, Category = "AI Performance", meta = (AllowPrivateAccess = "true"))
