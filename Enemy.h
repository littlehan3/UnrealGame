...
public:
    ...
    float MaxHealth = 100.0f; // MaxHealth (기존 Health를 CurrentHealth로 사용)
...
private:
    UPROPERTY()
    class AEnemyAIController* AICon; // AI 컨트롤러 캐싱을 위한 참조

    UPROPERTY()
    class UEnemyAnimInstance* AnimInstance; // 애니메이션 인스턴스 캐싱을 위한 참조

    UPROPERTY()
    class UCharacterMovementComponent* MoveComp; // 무브 컴포넌트 캐싱을 위한 참조

    UPROPERTY(EditAnywhere, Category = "EnemyType", meta = (AllowPrivateAccess = "true"))
    ...
    UPROPERTY(EditDefaultsOnly, Category = "VisualEffects", meta = (AllowPrivateAccess = "true"))
    ...
    UPROPERTY()
	UAnimMontage* LastPlayedJumpAttackMontage; // 마지막으로 재생된 점프 공격 몽타주 참조

	void ApplyEliteSettings(); // 엘리트 적 설정 적용
